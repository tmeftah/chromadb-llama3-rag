...
def load_models(model_name="unsloth/Llama-3.2-3B-Instruct", embed_model_name="sentence-transformers/all-MiniLM-L6-v2"):
    ...
    embed_model = AutoModel.from_pretrained(embed_model_name).to("cuda")  # Move embedding model to CUDA
...
def get_embeddings(texts, embed_tokenizer, embed_model):
    ...
    inputs = embed_tokenizer(texts, padding=True, truncation=True, return_tensors="pt").to("cuda") # Move inputs to CUDA
    with torch.no_grad():
        embeddings = embed_model(**inputs).last_hidden_state.mean(dim=1)
    return embeddings.cpu()  # Move embeddings back to CPU
...
def main(
    ...
    if torch.cuda.is_available():
        device = "cuda"
        console.print("[bold green]CUDA is available! Using GPU.[/bold green]")
    else:
        device = "cpu"
        console.print("[bold yellow]CUDA is not available. Using CPU.[/bold yellow]")
...
"""
                    ...
                    system_message = "You are a helpful assistant that only responds with working day estimates in the exact format: 'Total Working Days : X days'"
                    ...
                    result = qa_model(messages, temperature=0.1)
